...
void main() {
  group('the gate comes first', () {
    test('a file the server did not mark searchable never appears', () async {
      ...
      final results = await TalentSearchRepository(
        firestore: firestore,
  ...
  group('filtering by position', () {
    test('it returns any of the requested positions', () async {
      ...
      final results = await TalentSearchRepository(firestore: firestore).search(
      ...
      expect(results.map((user) => user.uid).toSet(), <String>{'cb', 'lb'});
    });
  ...
  group('filtering by age bracket', () {
    test('the bounds are inclusive', () async {
      ...
      expect(results.map((user) => user.uid).toSet(), <String>{
        'y2006',
        'y2008',
      });
    });
    ...
    test('a dual national is found under either passport', () async {
      ...
      for (final code in <String>['CI', 'FR']) {
        final results = await repository.search(
          TalentSearchQuery(nationality: code),
        );
  ...
  group('blocked pairs are excluded', () {
    test('a blocked uid is removed from an already-fetched page', () async {
      // Same shape as the nationality filter above: applied to the page
      // already fetched, never turned into a second Firestore query.
      final firestore = FakeFirebaseFirestore();
      await _addPlayer(firestore, 'visible', isSearchable: true);
      await _addPlayer(firestore, 'blocked', isSearchable: true);

      final results = await TalentSearchRepository(
        firestore: firestore,
      ).search(const TalentSearchQuery(), excludedUids: <String>{'blocked'});

      expect(results.map((user) => user.uid), <String>['visible']);
    });

    test('an empty exclusion set changes nothing', () async {
      final firestore = FakeFirebaseFirestore();
      await _addPlayer(firestore, 'a', isSearchable: true);
      await _addPlayer(firestore, 'b', isSearchable: true);

      final results = await TalentSearchRepository(
        firestore: firestore,
      ).search(const TalentSearchQuery());

      expect(results.map((user) => user.uid).toSet(), <String>{'a', 'b'});
    });
  });
  ...
  group('the query stays within what an index can serve', () {
    test('a page is bounded', () async {
      ...
      for (
        var index = 0;
        index < TalentSearchRepository.pageSize + 5;
        index++
      ) {
